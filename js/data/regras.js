...
window.REGRAS = (function () {
  ...
  // Requerimentos de técnicas: os 5 elementos, todas as habilidades inatas, todos os clãs e os 8 portões.
  // [grupo, nome, expressão que reconhece o texto do livro de jutsus]
  const REQUERIMENTOS = [];
  [['Katon', 'katon'], ['Suiton', 'suiton'], ['Fuuton', 'f[uū]u?ton'], ['Raiton', 'raiton'], ['Doton', 'doton']]
    .forEach(([n, r]) => REQUERIMENTOS.push(['Elemento', 'Elemento ' + n, new RegExp('^(elemento\\s+)?' + r + '$', 'i')]));
  const RE_INATA = {
    seis_bracos: /6 bra[cç]os/i, aojorou: /aojorou/i, bakuton1: /bakuton\s*i\b(?!i)/i, bakuton2: /kibaku\s*nendo|bakuton\s*ii/i,
    bolha: /bolha de sab/i, bolsa_veneno: /bolsa de veneno/i, choujuu: /choujuu/i, cobra: /^(habilidade inata\s+)?cobra$/i,
    curandeiro: /curandeiro/i, futton: /futton/i, borracha: /homem de borracha/i, hyouton: /hyouton/i, jashin: /jashin/i,
    jinton: /jinton/i, jiton1: /jiton\s*i\b(?!i)|^jiton$/i, bencao_areias: /ben[cç][aã]o das areias/i, jiton2: /jiton\s*ii/i,
    jiongu: /^(kinjutsu\s+|habilidade inata\s+)?jiongu$/i, kami: /\bkami\b/i, kyoumeisen: /kyoumeisen/i,
    mokuton: /mokuton kekkei|^mokuton$/i, punhos: /punhos b/i, ranton: /ranton/i, roubo: /roubo de chakra/i,
    sensorial: /sensorial$/i, shakuton: /shakuton/i, shikotsu: /shikotsumyaku/i, senninka: /senninka/i, siameses: /siames/i,
    tubarao: /tubar[aã]o/i, tubos: /tubos de ar/i, youton1: /youton\s*i\b(?!i)/i, youton2: /youton\s*ii\b(?!i)/i, youton3: /youton\s*iii/i,
  };
  INATAS.forEach((h) => REQUERIMENTOS.push(['Habilidade inata', h.nome, RE_INATA[h.id] || new RegExp('^' + h.nome + '$', 'i')]));
  const RE_CLA = {
    aburame_kikai: /kikaich/i, aburame_rinkai: /rinkaich/i, akimichi: /akimichi/i, hoozuki: /hoozuki/i, hyuuga: /hyuuga/i,
    inuzuka: /^(cl[aã]\s+)?inuzuka$/i, nara: /^(cl[aã]\s+)?nara$/i, sarutobi: /sarutobi/i, senju: /senju/i, uchiha: /uchiha/i,
    uzumaki: /uzumaki/i, yamanaka: /yamanaka/i,
  };
  CLAS.filter((c) => RE_CLA[c.id]).forEach((c) => REQUERIMENTOS.push(['Clã', 'Clã ' + c.nome, RE_CLA[c.id]]));
  PORTOES.forEach((p, i) => REQUERIMENTOS.push(['Portões', p[0].split(':')[0], new RegExp('^' + (i + 1) + 'º port', 'i')]));
